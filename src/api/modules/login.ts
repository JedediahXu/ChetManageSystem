...
export const loginApi = (params: Login.ReqLoginForm) => {
	return http.post<Login.ResLogin>(`/api` + PORT1 + `/login`, params);
	return http.post<Login.ResLogin>(`/api` + PORT1 + `/login`, {}, { params }); // post 请求携带 query 参数  ==>  ?username=admin&password=123456
	return http.post<Login.ResLogin>(`/api` + PORT1 + `/login`, qs.stringify(params)); // post 请求携带 表单 参数  ==>  application/x-www-form-urlencoded
...
export const getMenuList = () => {
	return http.get<Menu.MenuOptions[]>(`/api` + PORT1 + `/menu/list`);
};
